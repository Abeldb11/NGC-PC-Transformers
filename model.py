...
def _patch_jax_component_serialization():
    """
    Root-cause fix, applied once, globally: patches JaxComponent.save()/
    .load() (the base class every ngclearn component inherits its default
    serialization from) so that ANY compartment value which can't be
    directly converted to a plain array -- not just Adam's specific
    opt_params shape, but any nested list/tuple of differently-shaped
    arrays, from any optimizer, on any component -- gets safely flattened
    before saving and reconstructed on load, instead of crashing inside
    jnp.savez's generic np.asanyarray() conversion.

    Compartments that already save fine (plain arrays, or lists/tuples of
    *same*-shaped arrays that numpy can already stack) are left on the
    original fast path, unchanged -- this only intervenes for values that
    would otherwise crash.

    Because this patches the shared base class rather than individual
    component instances, no per-component wiring is needed anywhere,
    now or for any component added in the future.
    """
...
# Applied once, at import time -- every component constructed anywhere in
# this file (current and future) automatically gets safe save/load.
